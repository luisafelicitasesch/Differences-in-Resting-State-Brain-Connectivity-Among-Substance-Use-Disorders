...
% Define the group names based on groupCodes
groupNames = {'Controls', 'Nicotine', 'Cocaine'};

% Initialize variables to store AUC values for each measure and each group
Cglob_AUC = struct('Controls', [], 'Nicotine', [], 'Cocaine', []);
Tglob_AUC = struct('Controls', [], 'Nicotine', [], 'Cocaine', []);
Eglob_AUC = struct('Controls', [], 'Nicotine', [], 'Cocaine', []);
Pglob_AUC = struct('Controls', [], 'Nicotine', [], 'Cocaine', []);
...
for p = 1:numParticipants
    ...
    groupCode = groupCodes(p);
    if groupCode < 1 || groupCode > 3
        error('Invalid group code: %d', groupCode);
    end
...
numNodes = size(local_measures{1}.Clustering, 1); % Number of nodes

% Define the group names based on groupCodes
groupNames = {'Controls', 'Nicotine', 'Cocaine'};

% Initialize structures to store AUC values for each measure and each group
Cloc_AUC = struct('Controls', [], 'Nicotine', [], 'Cocaine', []);
Eloc_AUC = struct('Controls', [], 'Nicotine', [], 'Cocaine', []);
BCloc_AUC = struct('Controls', [], 'Nicotine', [], 'Cocaine', []);
ECloc_AUC = struct('Controls', [], 'Nicotine', [], 'Cocaine', []);
Dloc_AUC = struct('Controls', [], 'Nicotine', [], 'Cocaine', []);
